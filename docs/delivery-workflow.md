...
- A self-contained local acceptance review that checks source traceability, navigation, keyboard use, and narrow screens without recruited readers, outside reviewers, hosted previews, or third-party services.
...
Use the pilot review to settle schema and authoring friction, then build toward the proposal's initial release target: six reviewed competition guides, at least twelve solution records, eight practice guides, and the beginner-to-advanced path. Grow in source-qualified batches; keep the completeness levels honest and let missing task coverage and documented source gaps determine the order. See [proposal.md](proposal.md) for the acceptance gates and deferred work.
...
The Luna review is a Codex subagent review step, not a third-party account assignment. The Codex agent may merge only after the exact-head approval and required local/CI checks pass. Content publication follows the same evidence and exact-head review gate in [editorial-workflow.md](editorial-workflow.md); no separate human approval or participant study is required.
...
- Content evidence-review identity/date and the exact GPT-6 Luna Max subagent verdict and reviewed commit for publishable changes.
