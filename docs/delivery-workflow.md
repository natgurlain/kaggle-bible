...
4. Run the checks required by the ticket. For visible site changes, include a preview link and review the affected page at desktop and narrow widths.
...
6. Merge the ticket PR into `epic/evidence-pilot` only after the Luna Max review has no unresolved blocking findings and required checks pass. Update the epic checklist. Keep issue references as `Refs` on these PRs; close the child issues in the final PR to `main` so their status reflects the delivered default-branch state.
...
- Run the full content checks and production build on the final epic head. Review the Vercel preview and confirm the catalog-to-guide and practice-to-evidence paths.
- Have GPT-6 Luna at Max reasoning review the exact final diff, including integration effects across tickets. Rerun the review if the epic branch changes afterward.
- Merge the epic PR only after required checks pass and no blocking review findings remain. Then verify the production deployment corresponds to the merged `main` commit and close the epic.
...
- Commands/checks and their results; for UI changes, a preview and pages reviewed.
- GPT-6 Luna model, Max reasoning effort, reviewed commit SHA, verdict, and remaining findings.
