...
Common published content fields: `schema_version`, `id`, `status`, `title`, `summary`, `reviewed_by`, and `reviewed_at`. These record evidence/content review and may identify an AI reviewer. Publication additionally requires a separate human editorial approval receipt: `editorial_approval_type: human`, `editorial_approved_by`, and `editorial_approved_at`. Model review metadata alone cannot publish content. Sources and reproductions use their own status fields described below.
