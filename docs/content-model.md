...
Common published content fields: `schema_version`, `id`, `status`, `title`, `summary`, `reviewed_by`, and `reviewed_at`. These record evidence/content review; this project uses GPT-6 Luna Max Codex subagent review on the exact pull-request head. Publication requires valid evidence review, complete required fields and references, and published status. There is no separate human-approval receipt. Sources and reproductions use their own status fields described below.
...
Use body markers such as `[claim:gain-01]` immediately after substantive claims. The Sätteri Markdown plugin turns them into links to the guide's evidence cards, and `pnpm check-content` rejects markers without a matching local claim. Cross-record evidence links live in frontmatter as canonical references. Keep source-based facts such as data counts in claims too; a bibliography alone is insufficient.

Public competition guide routes are generated only for reviewed Level 2 evidence maps and Level 3 guides. Level 1 records stay in the catalog and do not receive a guide page.
...
A search result or index is a discovery lead. It is not sufficient evidence for a detailed solution claim. `content_reviewed: true` means the relevant content was inspected; it does not itself mean an independent reproduction occurred. Sources behind login remain explicitly access-limited and are not treated as inspected evidence unless their relevant contents are available for review.
...
The pull-request workflow installs the exact checked-in pnpm version, uses Node 22.12.0 (the repository's declared minimum), installs with a frozen lockfile, and runs these checks without Kaggle credentials, network scraping, or paid services. Automated checks establish structural consistency; the GPT-6 Luna Max Codex subagent checks whether claims, rankings, metric interpretation, applicability, contradictions, and source attribution are supported on the exact PR head. No participant study, outside reviewer, paid service, or hosted deployment is required. External link checks remain separate future work; a 403 or rate limit is not a reason to delete historical evidence or block every unrelated PR.
